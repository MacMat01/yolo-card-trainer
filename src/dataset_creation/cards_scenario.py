...
decalX = int((imgW - cardWidth) / 2)
decalY = int((imgH - cardHeight) / 2)

# Scenario with 3 cards : decal values are different
decalX3 = int(imgW / 2)
decalY3 = int(imgH / 2 - cardHeight / 2)
...
cardKP = ia.KeypointsOnImage([
    ...
    ia.Keypoint(x=decalX + cardWidth, y=decalY),
    ia.Keypoint(x=decalX + cardWidth, y=decalY + cardHeight),
    ia.Keypoint(x=decalX, y=decalY + cardHeight)
], shape=(imgH, imgW, 3))
...
class Scene:
    ...
    def create2CardsScene(self, bg, img1, class1, hulla1, hullb1, img2, class2, hulla2, hullb2):
        ...
        img1 = cv2.resize(img1, (cardWidth, cardHeight))
        self.img1 = np.zeros((imgH, imgW, 4), dtype=np.uint8)
        self.img1[decalY:decalY + cardHeight, decalX:decalX + cardWidth, :] = img1
        ...
        while True:
            self.listbba = []
            img2 = cv2.resize(img2, (cardWidth, cardHeight))
            self.img2 = np.zeros((imgH, imgW, 4), dtype=np.uint8)
            self.img2[decalY:decalY + cardHeight, decalX:decalX + cardWidth, :] = img2
    ...
    def create3CardsScene(self, bg, img1, class1, hulla1, hullb1, img2, class2, hulla2, hullb2, img3, class3, hulla3,
        ...
        img1 = cv2.resize(img1, (cardWidth, cardHeight))
        img2 = cv2.resize(img2, (cardWidth, cardHeight))
        img3 = cv2.resize(img3, (cardWidth, cardHeight))
        self.img3 = np.zeros((imgH, imgW, 4), dtype=np.uint8)
        self.img3[decalY3:decalY3 + cardHeight, decalX3:decalX3 + cardWidth, :] = img3
        self.img3, self.lkps3, self.bbs3 = augment(self.img3, [cardKP, kpsa3, kpsb3], trans_rot1)
        self.img2 = np.zeros((imgH, imgW, 4), dtype=np.uint8)
        self.img2[decalY3:decalY3 + cardHeight, decalX3:decalX3 + cardWidth, :] = img2
        self.img2, self.lkps2, self.bbs2 = augment(self.img2, [cardKP, kpsa2, kpsb2], trans_rot2)
        self.img1 = np.zeros((imgH, imgW, 4), dtype=np.uint8)
        self.img1[decalY3:decalY3 + cardHeight, decalX3:decalX3 + cardWidth, :] = img1
